        ...
        try {
            try {
                ...
                while (it.hasNext()) {
                    ...
                    // 条件成立：物理节点 角色 发生变化了，这个时候需要将它从 broker物理节点map中 移除。
                ...
                // 条件成立：broker上的主题不为Null，并且 当前物理节点 是 broker上的master节点。
                if (null != topicConfigWrapper
                    ...
                    if (this.isBrokerTopicConfigChanged(brokerAddr, topicConfigWrapper.getDataVersion())
                        || registerFirst) {
                        // 获取当前broker信息 中的 主题 映射表，从broker注册数据获取。
                        ConcurrentMap<String, TopicConfig> tcTable =
                            topicConfigWrapper.getTopicConfigTable();

                        if (tcTable != null) {
                            // 加入 或者 更新到 nameserver内
                ...
                // 添加 当前 节点的 brokerLiveInfo ，定时任务 会 扫描 这个映射表
                // 返回上一次 心跳时 当前 broker节点的 存活对象数据
