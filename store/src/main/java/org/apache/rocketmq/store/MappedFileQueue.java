...
public class MappedFileQueue {
        ...
        if (files != null) {
            ...
            for (File file : files) {
                ...
                try {
                    ...
                    // 设置wrotePosition 和 flushedPosition  这里给的值 都是 mappedFileSize，并不是准确值，准确值 需要 recover阶段设置。
                    mappedFile.setWrotePosition(this.mappedFileSize);
                    mappedFile.setFlushedPosition(this.mappedFileSize);
                    mappedFile.setCommittedPosition(this.mappedFileSize);

                    // 将当前 file的mappedFile对象 加入到 list集合管理。
     ...
     * @param cleanImmediately  true 强制删除 不考虑 expiredTime 这个条件了
     */
    ...
    public int deleteExpiredFileByOffset(long offset, int unitSize) {
        ...
        if (null != mfs) {
            ...
            for (int i = 0; i < mfsLength; i++) {
                ...
                if (result != null) {
                    // 获取cqData.msgPhyOffset
                    long maxOffsetInLogicQueue = result.getByteBuffer().getLong();
                    result.release();

                    // true，说明 当前mf内全部的 cqData 都是过期数据。
    ...
    public MappedFile findMappedFileByOffset(final long offset, final boolean returnFirstOnNotFound) {
        try {
            ...
            if (firstMappedFile != null && lastMappedFile != null) {
                // 条件成立：说明offset 没能命中 list内的mappedFile
                if (offset < firstMappedFile.getFileFromOffset() || offset >= lastMappedFile.getFileFromOffset() + this.mappedFileSize) {
                    ...
                    // 正常在这里返回，条件成立 说明 mappedFile内包含 offset 偏移量
