...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public DefaultMQProducerImpl(final DefaultMQProducer defaultMQProducer, RPCHook rpcHook) {
        ...
        // 参数4：时间单位
        // 参数5：异步消息线程池任务队列
        // 参数6：线程工厂，创建出来的线程 AsyncSenderExecutor_ 前缀
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        // 从生产者本地 主题发布信息 映射表 尝试获取 主题发布信息
        TopicPublishInfo topicPublishInfo = this.topicPublishInfoTable.get(topic);

        // 条件成立：说明 生产者本地保存的 指定 topic 的发布信息 是空的，需要从 客户端 获取 主题发布信息。
        if (null == topicPublishInfo || !topicPublishInfo.ok()) {
            // 向生产者本地 保存一份 该topic 的空的 发布信息
            this.topicPublishInfoTable.putIfAbsent(topic, new TopicPublishInfo());
            // 让客户端从nameserver更新 该 topic 的路由数据
            this.mQClientFactory.updateTopicRouteInfoFromNameServer(topic);
            // 正常情况，这一步 再拿到的 主题发布信息  是有数据的！
            // 特殊情况，因为 namserver上 并没有 topic 的路由数据，所以 拿到的 topicPublishInfo 还是一个 空对象。
